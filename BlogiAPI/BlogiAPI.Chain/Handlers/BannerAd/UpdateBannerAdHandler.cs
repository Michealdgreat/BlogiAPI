...
{
    public class UpdateBannerAdHandler(BannerAdCommandService bannerAdCommandService) : Handler<ICommandBase>
    {
        ...
        public override async Task<OperationResult> HandleRequest(ICommandBase request)
        {
            try
            {
                if (request is UpdateBannerAdCommand command)
                {
                    var result = await _bannerAdCommandService.UpdateBannerAd(command);
                    return result;
                }
            }
            catch (Exception)
