...
{
    public class DeletePostHandler(PostCommandService postCommandService) : Handler<ICommandBase>
    {
        ...
        public override async Task<OperationResult> HandleRequest(ICommandBase request)
        {
            try
            {
                if (request is DeletePostCommand command)
                {
                    var result = await _postCommandService.DeletePost(command);
                    return result;
                }
            }
            catch (Exception)
